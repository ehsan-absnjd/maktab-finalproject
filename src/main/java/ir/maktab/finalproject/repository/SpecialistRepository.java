...
public interface SpecialistRepository extends JpaRepository<Specialist, Long> {
    List<Specialist> findByFirstName(String firstName, Pageable pageable);

    List<Specialist> findByLastName(String lastName, Pageable pageable);

    List<Specialist> findByEmail(String email, Pageable pageable);

    @Query(value = "SELECT s FROM Specialist s where :assistance MEMBER OF s.assistances ORDER BY s.id DESC")
    List<Specialist> findByAssistanceId(Assistance assistance , Pageable pageable);
